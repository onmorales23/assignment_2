...
#When using the test data set and fitting the important variables,
 #the conditional ATE was small, which ranges from -0.4 to 0.2 with a mean of 0.018.
...
#Predictor 1
groupX1MTHETK1 <- quantile(train_data[, names(Top2predictors)[1]])
train_data$groupX1M <- ifelse(train_data[,names(Top2predictors)[1]] >= groupX1MTHETK1[4], 4, 
                             ifelse(train_data[,names(Top2predictors)[1]]>=groupX1MTHETK1[3] & train_data[,names(Top2predictors)[1]]<groupX1MTHETK1[4], 3,
                                    ifelse(train_data[,names(Top2predictors)[1]]>=groupX1MTHETK1[2] & train_data[,names(Top2predictors)[1]]<groupX1MTHETK1[3], 2,
                                           1)))
train_data$groupX1M <- factor(train_data$groupX1M)
boxplot(CATE_causalForest ~ train_data$groupX1M, xlab = paste(names(Top2predictors)[1], "Group"), ylab = "CATE")

#Predictor 2
groupX1R <- quantile(train_data[, names(Top2predictors)[2]])
train_data$groupX1R <- ifelse(train_data[,names(Top2predictors)[2]] >= groupX1R[4], 4, 
                             ifelse(train_data[,names(Top2predictors)[2]]>=groupX1R[3] & train_data[,names(Top2predictors)[2]]<groupX1R[4], 3,
                                    ifelse(train_data[,names(Top2predictors)[2]]>=groupX1R[2] & train_data[,names(Top2predictors)[2]]<groupX1R[3], 2,
                                           1)))
train_data$groupX1R <- factor(train_data$groupX1R)
boxplot(CATE_causalForest ~ train_data$groupX1R, xlab = paste(names(Top2predictors)[2], "Group"), ylab = "CATE")
...
Causal Forest (CF) did not fit very well with the data set. Because when using the training data set, CF with the best tunning parameters detected heterogeneity, but it failed to detect heterogeneity by using the testing data set. When plotting the top two important variables with CATE, we found a small difference between groups.

In summary, this paper attempeted to explore treatment effect heterogeneity with Causal Forests, GenericML, and Causal BART. Unfortunately, there was no significant heterogeneity to be found in terms of math score outcome. All three methods came to similar conclusions.
